...
class Menu(BaseScene):
    ...
    def __init__(self, screen: Surface, can_resume: bool, *args, **kwargs) -> None:
        ...
        frame = self.menu.add.frame_v(525, 500)
        if self.can_resume:
            ...
            frame.pack(
                self.menu.add.button("Quit Level", GameEvent(EventType.SHOW_MENU_AND_RESET_LEVEL_ID).post)
            )

        available_level_ids = util.get_available_level_ids()

        normal_level_ids = [level_id for level_id in available_level_ids if 1 <= level_id <= 10]
        frame.pack(
            self.menu.add.dropselect(
                title="Play Level...",
                items=[(str(level_id), level_id) for level_id in normal_level_ids],
        ...
        frame.pack(self.menu.add.button("Quit Game", lambda: GameEvent(pygame.QUIT).post()))
